...
/*   Updated: 2018/03/06 17:47:20 by mfrisby          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			init_space(t_cam *cam, t_pix **pix, t_upleft **upleft)
{
	...
	if (((*upleft) = malloc(sizeof(t_upleft))) == NULL)
	{
		ft_printf("\033[0;31mError: %s\033[0m", strerror(errno));
		exit(0);
	}
	if (((*pix) = malloc(sizeof(t_pix))) == NULL)
	{
		ft_printf("\033[0;31mError: %s\033[0m", strerror(errno));
		exit(0);
	}
	(*upleft)->x = cam->camx + ((cam->vdirx * cam->fovd) + (cam->vupx
				* (cam->fovh / 2.0f))) - (cam->vrightx * (cam->fovw / 2.0f));
	(*upleft)->y = cam->camy + ((cam->vdiry * cam->fovd) + (cam->vupy

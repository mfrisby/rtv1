...
float    calcul_cone(t_cam *cam, t_ray *ray, t_cone *cone)
{
    ...
    b = 2 * (ray->x * (cam->camx - cone->x) + ray->z * (cam->camz - cone->z) - ray->y * (cam->camy - cone->y) * cone->r);
    c = (pow(cam->camx - cone->x, 2) + pow(cam->camz - cone->z, 2) - pow(cam->camy - cone->y, 2) * cone->r);
    delta = (b * b) - 4 * a * c;
    b = -b;
    if (delta > 0.0f)
    {
